...
치지직/인터넷방송 시청 취향을 14개 문항으로 분석해 `data/streamers.json`에서 가장 잘 맞는 스트리머를 추천하는 정적 웹앱입니다. 외부 서버나 프레임워크 없이 HTML, CSS, JavaScript만 사용하며 GitHub Pages에서 바로 동작합니다.

## 주요 기능

- 버튜버 포함/제외 필터 및 쉽게 변경 가능한 듀라한 포함 옵션
- 598명 데이터 기반 검색형 최애 스트리머 선택
- 9개 취향 축, 주력 콘텐츠, 방송 스타일과 새 최애 매력을 이용한 14문항 성향 테스트
- 정확도와 다양성을 반영한 TOP 3와 관계·컨텐츠·입맛·발굴 역할로 분리된 추가 추천 4명
- 답변·최애·버튜버 설정의 stable hash로 재현 가능하게 회전하는 롱테일 discovery 슬롯
- 좌·중·우 TOP 3 결과 카드, 라이브 방송 화면, 가상 채팅 UI
...
`data/streamers.json`은 기존 배열형과 새 그래프형을 모두 지원합니다.
...
```json
{ "metadata": { "edge_mode": "undirected_deduplicated" }, "nodes": [...], "edges": [...] }
```

그래프형의 `nodes`는 앱 로드 시 기존 추천 로직이 사용하는 한국어 필드와 9축 지표로 normalize됩니다. `edges`는 ID 또는 이름으로 노드를 찾아 `연결관계`로 복원되며, 원본 JSON 파일 자체는 변경하지 않습니다. 앱은 다음 호환 필드를 사용합니다.
