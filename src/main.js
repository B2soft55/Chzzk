...
/** TOP 3는 정확도와 다양성을 유지하고 추가 추천은 관계·컨텐츠·입맛·발굴 슬롯으로 분리합니다. */
function recommendations(){
  ...
  const scored=pool.map(s=>{const test=similarity(vector,streamerVector(s)),game=gamePreferenceScore(s,gameKeywords),preference=preferenceScore(s,preferenceKeywords),taste=blendedTasteScore(test,game,preference),fav=favVector?similarity(favVector,streamerVector(s)):0,relation=favoriteRelations.get(nameOf(s)),score=taste*(1-fw)+fav*fw,rankScore=relationshipRankScore(score,relation,FAVORITE_RELATION_MULTIPLIER),relationBonus=Math.max(0,rankScore-score)*.10,diversityScore=((s?.규모티어==='longtail'||s?.scale_tier==='longtail')?longtailWeight*.08:0)+(stringList(s?.추천풀||s?.recommendation_pool).includes('hidden_gem')?exploreWeight*.05:0),diversityBonus=diversityScore*.05,finalScore=score*.85+relationBonus+diversityBonus;return {s,score,testScore:test,gameScore:game,preferenceScore:preference,rankScore,relationDepth:relation?.depth||null,finalScore,relationBonus,diversityBonus};}).sort((a,b)=>b.score-a.score||nameOf(a.s).localeCompare(nameOf(b.s)));
  if(!scored.length)return [];const sortedForBest=[...scored].sort((a,b)=>b.finalScore-a.finalScore||nameOf(a.s).localeCompare(nameOf(b.s))),bestTop=sortedForBest[0]?.finalScore??0,topPool=sortedForBest.filter(item=>item.finalScore>=bestTop-.03).slice(0,5),offset=stableHash(answerSignature())%topPool.length,rotatedTopPool=[...topPool.slice(offset),...topPool.slice(0,offset)],topPicks=[...rotatedTopPool,...sortedForBest.filter(item=>!topPool.includes(item))].slice(0,3).map(item=>({...item,slotRole:'best'})),winner=topPicks[0],winnerRelations=relationshipNeighborhood(nameOf(winner.s)),selected=new Set(topPicks.map(item=>nameOf(item.s))),next=[];
  const enriched=scored.filter(item=>!selected.has(nameOf(item.s))).map(item=>{const relation=winnerRelations.get(nameOf(item.s)),activeRelation=favoriteRelations.size?favoriteRelations.get(nameOf(item.s)):relation,tagBonus=similarTagRelation(favorite||winner.s,item.s)*SIMILAR_TAG_RELATION_BONUS;return {...item,winnerRelationDepth:relation?.depth||null,nextScore:relationshipRankScore(item.score,activeRelation,1+relationWeight)+tagBonus};});
  ...
  return [...topPicks,...next.slice(0,4)];
}
...
/** TOP 3 추천 카드를 좌·중·우 영역에 표시합니다. */
function topPickCard(item,i){const s=item.s,pct=Math.round(item.score*100);return `<article class="top-pick-card" data-rank="0${i+1}"><span class="rank-label">TOP ${i+1} 추천</span><h3 class="winner-name">${esc(nameOf(s))}</h3><span class="category">${esc(s['주력/종합게임']||contentLabel(s?.콘텐츠태그||s?.content_tags)||'종합 방송')}</span><div class="score-ring" style="--score:${pct}%"><span>${pct}%</span></div></article>`;}
function resultCard(item,i){const s=item.s,pct=Math.round(item.score*100),relation=state.favorite&&item.relationDepth?`<span class="tag relation-tag">최애와 관계 ${item.relationDepth}단계</span>`:item.winnerRelationDepth?`<span class="tag relation-tag">TOP 추천과 관계 ${item.winnerRelationDepth}단계</span>`:'',role={relation:'최애와 관련이 있는',content:'컨텐츠가 비슷한',taste:'입맛이 비슷한',discovery:'이런 스트리머는 어떤가요?'}[item.slotRole]||'추가 추천';return `<article class="recommend-card"><span class="recommend-rank">0${i+4} · ${role}</span><h3 class="recommend-name">${esc(nameOf(s))}</h3><div class="tags"><span class="tag">${esc(s.버튜버여부||'스트리머')}</span><span class="tag">${esc(s['주력/종합게임']||contentLabel(s?.콘텐츠태그||s?.content_tags)||'종합 방송')}</span>${relation}</div><span class="match-percent">MATCH ${pct}%</span></article>`;}
...
function fallbackResultCanvas(source){
  const canvas=document.createElement('canvas'),width=1080,pad=72,topCards=Array.from(source.querySelectorAll('.top-pick-card')),cards=Array.from(source.querySelectorAll('.recommend-card'));canvas.width=width;canvas.height=1040+cards.length*135;const context=canvas.getContext('2d');if(!context)throw new Error('Canvas를 사용할 수 없음');
  ...
  context.fillStyle='#fff';context.font='900 58px system-ui, sans-serif';wrapCanvasText(context,source.querySelector('.result-type')?.textContent,pad,170,width-pad*2,68,2);context.fillStyle='#9baba3';context.font='24px system-ui, sans-serif';context.fillText('당신의 취향에 맞는 TOP 3 방송을 찾았습니다.',pad,300);
  topCards.forEach((card,i)=>{const x=pad+i*312,y=355;context.fillStyle='#121a17';context.fillRect(x,y,292,260);context.fillStyle='#00ffa3';context.font='800 20px system-ui, sans-serif';context.fillText(`TOP ${i+1} 추천`,x+22,y+42);context.fillStyle='#fff';context.font='900 38px system-ui, sans-serif';wrapCanvasText(context,card.querySelector('.winner-name')?.textContent,x+22,y+105,245,43,2);context.fillStyle='#ff3d75';context.font='800 24px system-ui, sans-serif';context.fillText(card.querySelector('.score-ring span')?.textContent||'',x+22,y+225);});
  context.fillStyle='#fff';context.font='800 30px system-ui, sans-serif';context.fillText('추가 추천',pad,690);cards.forEach((card,i)=>{const y=730+i*135;context.fillStyle='#121a17';context.fillRect(pad,y,width-pad*2,105);context.fillStyle='#00ffa3';context.font='800 18px system-ui, sans-serif';context.fillText(card.querySelector('.recommend-rank')?.textContent||'',pad+28,y+42);context.fillStyle='#fff';context.font='800 30px system-ui, sans-serif';context.fillText(card.querySelector('.recommend-name')?.textContent||'',pad+360,y+45);context.fillStyle='#ff3d75';context.font='800 20px system-ui, sans-serif';context.fillText(card.querySelector('.match-percent')?.textContent||'',width-pad-170,y+45);});return canvas;
}
...
/** TOP 3와 관계·컨텐츠·입맛·발굴 추가 추천을 렌더링합니다. */
function renderResult(){
  setScene('LIVE 매칭 완료');const {vector}=buildUserVector(),results=recommendations(),topPicks=results.slice(0,3),winner=topPicks[0];
  ...
  const topNames=topPicks.map(item=>nameOf(item.s)),pct=Math.round(winner.score*100);
  app.innerHTML=`<section class="screen result-screen"><div class="result-header"><div><div class="eyebrow">Your streaming taste</div><h2 class="result-type">${esc(typeName(vector))}</h2><p class="screen-desc">당신의 취향 데이터를 기반으로 TOP 3 방송을 찾았습니다.</p></div><span class="match-stamp">● LIVE 매칭 완료</span></div><h3 class="top-picks-title">TOP 3 추천</h3><div class="result-main">${topPicks.map(topPickCard).join('')}</div><h3 class="recommend-title">다음 방송도 둘러보세요 · 추가 추천</h3><div class="recommend-grid">${results.slice(3).map(resultCard).join('')}</div><div class="action-row result-actions" data-capture-exclude><button class="primary-btn" id="save-image">결과 이미지 저장</button><button class="secondary-btn" id="copy">결과 이름 복사</button><button class="secondary-btn" id="reset">테스트 다시하기</button></div></section>`;
  document.querySelector('#reset').onclick=reset;document.querySelector('#save-image').onclick=saveResultImage;document.querySelector('#copy').onclick=async()=>{try{await navigator.clipboard.writeText(`${typeName(vector)} · TOP 3 추천 ${topNames.join(', ')}`);announce('결과를 클립보드에 복사했어요.');}catch{announce(`TOP 3 추천: ${topNames.join(', ')}`);}};
  addChat('매칭봇',`${topNames.join(', ')}님을 TOP 3로 추천! 1순위 매칭 ${pct}%`,true);
}
