...
function recommendations(){
  ...
  if(!scored.length)return [];const sortedForBest=[...scored].sort((a,b)=>b.finalScore-a.finalScore||nameOf(a.s).localeCompare(nameOf(b.s))),bestTop=sortedForBest[0]?.finalScore??0,topPool=sortedForBest.filter(item=>item.finalScore>=bestTop-.03).slice(0,5),winner={...topPool[stableHash(answerSignature())%topPool.length],slotRole:'best'},winnerRelations=relationshipNeighborhood(nameOf(winner.s)),selected=new Set([nameOf(winner.s)]),next=[];
  const enriched=scored.filter(item=>nameOf(item.s)!==nameOf(winner.s)).map(item=>{const relation=winnerRelations.get(nameOf(item.s)),activeRelation=favoriteRelations.size?favoriteRelations.get(nameOf(item.s)):relation,tagBonus=similarTagRelation(favorite||winner.s,item.s)*SIMILAR_TAG_RELATION_BONUS;return {...item,winnerRelationDepth:relation?.depth||null,nextScore:relationshipRankScore(item.score,activeRelation,1+relationWeight)+tagBonus};});
